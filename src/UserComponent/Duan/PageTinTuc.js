...
import SGD from './SGD'
import TTSK from './Tintuc'
export default function DsTTSK (){
...
useEffect(async()=>{
    if(!madm){
      try {
        const data = await axios.get(process.env.REACT_APP_API_KEY + "Listtintucsukien",null)
    ...
    <div className='my-carousel mt-4 listduan'>
       {language==0?<form className='search input-group' style={{margin:'0 auto',width:'99%', padding:' 0 10%'}} onFocus={()=>setIsSearch(true)} onBlur={()=>setIsSearch(false)}>
         <div style={{display:'flex',width:'100%', textAlign:'center', paddingLeft:'20%'}}>
          <input className='search-bar duan-input-search'  placeholder='Tìm Kiếm Theo Tiêu Đề Của Tin Tức & Sự Kiện.....' onChange={handleSearch} />
        ...
        {language==1?<form className='search input-group' style={{margin:'0 auto',width:'99%', padding:' 0 10%'}} onFocus={()=>setIsSearch(true)} onBlur={()=>setIsSearch(false)}>
         <div style={{display:'flex',width:'100%', textAlign:'center', paddingLeft:'20%'}}>
          <input className='search-bar duan-input-search'  placeholder='Search By Title Of News & Events.....' onChange={handleSearch} />
        ...
        <nav aria-label="Page navigation example">
          <ul className="pagination justify-content-center">
            <li className={p  <=0?"page-item disabled":'page-item'}>
              {language==0?<a className="page-link" href={'/pageTTSK/' +Number(p)} >Trang trước</a>:''}
              {language==1?<a className="page-link" href={'/pageTTSKEN/' +Number(p)} >PREV</a>:''}
            </li>
            {renderPage()}
            
            <li className={p>=pageNum -1?"page-item disabled":'page-item'}>
            {language==0?<a className="page-link" href={'/pageTTSK/' +Number(p+2)}>Trang tiếp</a>:''}
            {language==1?<a className="page-link" href={'/pageTTSKEN/' +Number(p+2)}>Next</a>:''}
            </li>
          </ul>
        </nav>
